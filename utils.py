...
# Analyze a time series by segmenting it and evaluating the trend of each segment.
# Returns 1 for uptrend, -1 for downtrend, and 0 when the change is within the delta threshold.
# Segments use length seg_size, comparing each segment's first and last values.

def y_trend_analyse(y, seg_size, delta):
    seg_no = int(np.floor(len(y)/seg_size))
    trends = []
    for s in range(seg_no):
        start = y[s*seg_size]
        end = y[(s+1)*seg_size-1]
        diff = end - start
        if diff > delta:
            trends.append(1)
        elif diff < -delta:
